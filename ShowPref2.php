                        ...
                        <div class="col-md-12">
                          <div class="home-box-content">
                            <div class="left-text">
                              ...
                              <p>The Following is  the List of Movies based on your choices. It is selected by finding nearest matches to your selected Movies.</p>
...
$sql = "SELECT * FROM movie_det where id in(".$ids.")";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
       ...
       $idarray[]=$row["id"];
       $poparray[]=$row["popularity"];
       $genrearr=explode(",", $row["genre"]);
       foreach ($genrearr as $val) {
         # code...
        if($val=="")
        {
          continue;
       ...
       $voterarray[]=$row["voter_average"];
       $votes[]=$row["vote_count"];
    }
}
$min_pop=min($poparray);
$min_rating=min($voterarray);
$min_vc=min($votes);
$n=sizeof($genrearray);
...
sort($genrearray); 
    ...
    for ($i = 1; $i < $n; $i++)  
    { 
        if ($genrearray[$i] == $genrearray[$i - 1]) 
            $curr_count++; 
        else 
        { 
   ...
   $sql = "SELECT * FROM movie_det where voter_average>".$min_rating." and vote_count>".$min_vc." and genre like '%".$res."%' and genre like '%".$res2."%' and popularity>".$min_pop."ORDER BY RAND()  LIMIT 15;";
